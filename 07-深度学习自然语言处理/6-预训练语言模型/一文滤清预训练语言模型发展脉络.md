...
![动图](./assets/v2-894de1f6545c5f98b44f0913e1001ed9_b.webp)



具体实现方式是，通过随机取一句话的一种排列，然后将末尾一定量的词给“遮掩”（和 BERT 里的直接替换 “[MASK]” 有些不同）掉，最后用 AR 的方式来按照这种排列依次预测被“遮掩”掉的词。

![img](./assets/v2-8ee6464348307333a9d14bb37a3ad073_1440w.webp)

我们可以发现通过随机取排列（Permutation）中的一种，就能非常巧妙地通过 **AR 的单向方式来习得双向信息**了。



**在 XLNet 中，尽管考虑了句子的所有可能排列，但实际输入到模型的句子顺序并没有改变。模型通过调整注意力矩阵来模拟不同的排列顺序，而不是直接改变输入句子的顺序。**
...
于是这里就造成了位置信息与内容信息的割裂，因此在 BERT 这样的位置信息加内容信息输入 Self-Attention (自注意力) 的流（Stream）之外，作者还增加了另一个**只有位置信息作为 Self-Attention 中 query 输入的流**。文中将前者称为 **Content Stream**，而后者称为 **Query Stream**。

这样就能利用 Query Stream 在对需要预测位置进行预测的同时，又不会泄露当前位置的内容信息。具体操作就是用两组隐状态（hidden states） 和 。其中 只有位置信息，作为 Self-Attention 里的 Q。 包含内容信息，则作为 K 和 V。具体表示如下图所示：

![img](./assets/v2-1054aa153cabdabd0361f069c76c4ddc_1440w.webp)
...
![img](./assets/v2-4844c76e051e3b7e00e9e603a8ce1d80_1440w.webp)

图：Content stream attention

于是组合起来就是这样：

![img](./assets/v2-8fdfceb19a40770998362d37ac3db12e_1440w.webp)
...
《Transformer-XL: Attentive Language Models Beyond a Fixed-Length Context》围绕如何建模长距离依赖，提出Transformer-XL（XL是extra long的意思）：

- 提出**片段级循环机制(segment-level recurrence mechanism)**，引入一个**记忆(memory)**模块（类似于cache或cell），循环用来建模片段之间的联系。这使得长距离依赖的建模成为可能；也使得片段之间产生交互，解决上下文碎片化问题。
- 提出**相对位置编码机制(relative position embedding scheme)**，代替绝对位置编码。在memory的循环计算过程中，避免时序混淆，位置编码可重用。
...
而事实证明， BERT-WWM 在中文任务上的确有着优势所在，具体就不细说了，至少目前来说，我们的中文预训练语言模型有三大选择了： BERT , ERNIE, 。
...
![动图](./assets/v2-88e4f76e9cab63dccf2122e70fb93b93_b.webp)
